...
def log_event(event_type, **details):
    """
    Forensic JSON-line logger.
    Includes timestamp, PID, event type, caller function, and arbitrary details.
    """
...
def getCSVData(dic_, dir_repo):
    log_event("ENTER_FUNCTION", dic_size=len(dic_), repo=dir_repo)

    temp_list = []
    for TEST_ML_SCRIPT in dic_:
        log_event("PROCESSING_FILE", file=TEST_ML_SCRIPT)
        ...
        try:
            ...
            log_event("ERROR_LINT_ENGINE", file=TEST_ML_SCRIPT, error=str(e))
            continue
        ...
        the_tup = (
            dir_repo, TEST_ML_SCRIPT, data_load_count, model_load_count,
            data_download_count, model_label_count, model_output_count,
    ...
    log_event("EXIT_FUNCTION", results=len(temp_list))
    return temp_list
  

def getAllPythonFilesinRepo(path2dir):
    log_event("SCAN_DIRECTORY", directory=path2dir)
        ...
        for file_ in filenames:
            ...
            if os.path.exists(full_path_file):
                if (file_.endswith(constants.PY_FILE_EXTENSION) and 
                    ...
                    log_event("PYTHON_FILE_FOUND", file=full_path_file)

    valid_list = np.unique(valid_list)
    log_event("SCAN_COMPLETE", count=len(valid_list))
    return valid_list


def runFameML(inp_dir, csv_fil):
    log_event("RUN_START", input_directory=inp_dir, output_csv=csv_fil)
    ...
    for subfolder in list_subfolders_with_paths:
        log_event("PROCESSING_SUBFOLDER", subfolder=subfolder)
    ...
    full_df = pd.DataFrame(df_list, columns=constants.CSV_HEADER)
    full_df.to_csv(csv_fil, header=constants.CSV_HEADER, index=False, encoding=constants.UTF_ENCODING)

    log_event("CSV_WRITTEN", rows=len(full_df), file=csv_fil)
...
if __name__=='__main__':
    ...
    try:
        ...
        if command_line_flag:
            ...
            log_event("USER_INPUT_PATH", path=dir_path)

            if os.path.exists(dir_path):
                repo_dir = dir_path
                output_csv = os.path.join(current_script_dir, "V5_OUTPUT_USER.csv")
                full_dict = runFameML(repo_dir, output_csv)
        else:
            log_event("AUTO_RUN_PATH", path=repo_dir)
            full_dict = runFameML(repo_dir, output_csv)

    except Exception as e:
        log_event("FATAL_ERROR", error=str(e))
        print("Fatal error during execution. See forensics.log for details.")
        raise
    ...
    log_event("PROGRAM_END", duration_minutes=time_diff)
